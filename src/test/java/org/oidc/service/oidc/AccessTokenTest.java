...
/**
 * Unit tests for {@link AccessToken}.
 */
public class AccessTokenTest extends BaseServiceTest<AccessToken> {
  ...
  public void init() {
    ...
    service = new AccessToken(serviceContext, state, null);
    ...
    resp.addClaim("code", "authorizationcode");
  ...
  public void testHttpGetParametersMinimal() throws Exception {
    ...
    Assert.assertTrue(httpArguments.getBody().contains("code=authorizationcode"));
    Assert.assertTrue(httpArguments.getBody().contains("grant_type=authorization_code"));
  }
  ...
  public void testdoUpdateServiceContextResponseStored()
    ...
    Message storedResponse = state.getItem(stateKey, MessageType.TOKEN_RESPONSE);
  ...
  public void testdoUpdateServiceContextWrongStatekey()
      throws MissingRequiredAttributeException, ValueException, InvalidClaimException {
    AccessTokenResponse response = new AccessTokenResponse();
    response.addClaim("token_type", "bearer");
    response.addClaim("access_token", "value");
    long expires = 3600;
    response.addClaim("expires_in", expires);
    service.updateServiceContext(response, "WrongStateKey");
  }
