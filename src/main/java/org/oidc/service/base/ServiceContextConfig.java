 ...
 * services because there is one ServiceContext that is instantiated based off this instance
 */
public class ServiceContextConfig {
  ...
  /**
   * Where the response to a dynamic client registration is stored. This is the unmodified
   * registration response as received from the authorization server.
   */
  ...
  /**
   * Some key material may be available to the client at initialization. This can either be from
   * local storage (files) or from a web service. If it’s on the web then the keySpecification will
   * be a MAP<string,string> where the key is the owner of the keys and the value is the URL where
   * the keys can be found. It is assumed that what is received when doing a HTTP GET on the url is
   * a JWKS. If it’s local the keySpecification will be a MAP<string,string> where the key is the
   * owner of the keys and the value is the path to a file containing a JWKS with the key
   * definitions.
   */
  private Map<FileOrUrl, KeySpecifications> keys;
  /**
   * A specification to be used when dynamically creating a set of keys at startup or when doing key
   * rotation. buildSpecification is a MAP<string, List<string>> if the keyDefs key is EC then the
   * buildSpecification keys are: ‘crv’ which elliptic curve that should be used, single valued
   * ‘use’ what the key can be used for; signing and/or encryption if the keyDefs key is RSA then
   * the buildSpecification keys are: ‘use’ what the key can be used for; signing and/or encryption
   * ‘name’ The name of the file where the key will be stored ‘path’ A path to the directory ‘size’
   * The key size (1024, 2048, 4096, ..)
   */
  private Map<KeyType, KeySpecifications> keyDefs;

  public ServiceContextConfig(ProviderConfigurationResponse providerConfigurationResponse,
      RegistrationResponse registrationResponse, String baseUrl, String requestsDirectory,
      Map<String, String> allow, RegistrationResponse behavior,
      RegistrationRequest registrationRequest, String clientId, String clientSecret, String issuer,
      List<String> redirectUris, Map<String, String> callBack,
  ...
  public static class ServiceContextConfigBuilder {
    ...
    public ServiceContextConfig.ServiceContextConfigBuilder setProviderInfo(
    ...
    public ServiceContextConfig.ServiceContextConfigBuilder setRequestsDirectory(
    ...
    public ServiceContextConfig.ServiceContextConfigBuilder setBehavior(
        RegistrationResponse behavior) {
      this.behavior = behavior;
      return this;
    }

    public ServiceContextConfig.ServiceContextConfigBuilder setRegistrationRequest(
    ...
    public ServiceContextConfig.ServiceContextConfigBuilder setRedirectUris(
        List<String> redirectUris) {
      this.redirectUris = redirectUris;
      return this;
    }

    public ServiceContextConfig.ServiceContextConfigBuilder setCallBack(
        Map<String, String> callBack) {
      this.callBack = callBack;
      return this;
    }

    public ServiceContextConfig.ServiceContextConfigBuilder setKeys(
        Map<FileOrUrl, KeySpecifications> keys) {
      this.keys = keys;
      return this;
    }

    public ServiceContextConfig buildServiceContext()
        throws NoSuchFieldException, IllegalAccessException {
      return new ServiceContextConfig(providerConfigurationResponse, registrationResponse, baseUrl,
          requestsDirectory, allow, behavior, registrationRequest, clientId, clientSecret, issuer,
          redirectUris, callBack, keys);
    }
  }
}
