...
/**
 * Commonly used claims. May need to be extended to accommodate additional claims
 */
public class AddedClaims implements Cloneable {
   ...
   *
   * The OAuth2 protocol is a delegated authorization mechanism, where an application requests
   * access to resources controlled by the user (the resource owner) and hosted by an API (the
   * resource server), and the authorization server issues the application a more restricted set of
   * credentials than those of the user.
   *
   * The scope parameter allows the application to express the desired scope of the access request.
   * In turn, the scope parameter can be used by the authorization server in the response to
   * indicate which scopes were actually granted (if they are different than the ones requested).
   */
  ...
  public static class AddedClaimsBuilder {
    ...
    public AddedClaimsBuilder setClientAuthenticationMethod(
    ...
    public AddedClaims buildAddedClaims() {
      return new AddedClaims(clientId, issuer, keyJar, shouldVerify, scope, resource,
          clientAuthenticationMethod, state);
    }
  }
}
